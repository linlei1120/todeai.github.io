...
2. **服务端凭证校验**：
   - 小程序将code发送到开发者服务器
   - 服务器使用code + AppID + AppSecret 请求微信接口
   - 微信返回openid（用户唯一标识）和session_key（会话密钥）
...
2. **登录态维护**：
   ...
   function secureRequest(options) {
     return new Promise((resolve, reject) => {
       ...
       if (!getApp().globalData.token) {
         getApp().silentLogin().then(makeRequest).catch(reject)
       } else {
         makeRequest()
       }
     })
...
2. **失败重试机制**：
   ```javascript
   // 带重试的静默登录
   async silentLoginWithRetry(retryCount = 2) {
     try {
       await this.silentLogin()
     } catch (error) {
       if (retryCount > 0) {
         return this.silentLoginWithRetry(retryCount - 1)
       }
       throw error
     }
   }
